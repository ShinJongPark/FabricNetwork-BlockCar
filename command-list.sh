...
# 생성된 체인코드 동작 테스트
docker exec -e "CORE_PEER_LOCALMSPID=carAssociationOrgMSP" -e "CORE_PEER_MSPCONFIGPATH=/var/hyperledger/users/msp" peer0.carAssociationOrg peer chaincode invoke -o orderer001:7050 -C carchannel001 -n car -c '{"Args":["registerCar","M12NAS22"]}'
...
docker exec -e "CORE_PEER_LOCALMSPID=carAssociationOrgMSP" -e "CORE_PEER_MSPCONFIGPATH=/var/hyperledger/users/msp" peer0.carAssociationOrg peer chaincode query -o orderer001:7050 -C carchannel002 -n car_002_management -c '{"Args":["getAccidentHistory","M12NAS22"]}'

# nodejs chaincode package
# 체인코드 수정 후 재배포를 위한 패키지 생성 명령어
# -v 플래그에 버전을 기록
docker exec -e "CORE_PEER_LOCALMSPID=carAssociationOrgMSP" -e "CORE_PEER_MSPCONFIGPATH=/var/hyperledger/users/msp" peer0.carAssociationOrg peer chaincode package /var/hyperledger/chaincode/ccpack.out -n car -v 0.2 -l node -p /var/hyperledger/chaincode/node
