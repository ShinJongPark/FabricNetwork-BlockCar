...
./configtxgen -profile CarOrdererGenesis -outputBlock orderer-genesis.block -channelID ordererchannel001
## 차량 시리얼 관리 채널
./configtxgen -profile CarSerialChannel -outputCreateChannelTx carchannel001.tx -channelID carchannel001
#3 차량 이력 관리 채널
./configtxgen -profile CarChannel -outputCreateChannelTx carchannel002.tx -channelID carchannel002
# 생성된 파일을 이동시킴(auctionchannel001.tx만 이동하며, orderer-genesis.block은 이동시키지 않음)
# 이후에 노드파일이 사용함
mv carchannel001.tx ./channel-artifacts
mv carchannel002.tx ./channel-artifacts
...
# check peer0.carAssociationOrg
nc -z 0.0.0.0 7051
if [ "$?" != 0 ]; then
    echo 'peer0.carAssociationOrg run fail. Please check docker log.'
else
    echo 'peer0.carAssociationOrg is active.'
fi

# check peer0.carInsuranceOrg
nc -z 0.0.0.0 8051
if [ "$?" != 0 ]; then
    echo 'peer0.carInsuranceOrg run fail. Please check docker log.'
else
    echo 'peer0.carInsuranceOrg is active.'
fi

# check peer0.carInsuranceOrg
nc -z 0.0.0.0 9051
if [ "$?" != 0 ]; then
    echo 'peer0.carMaintenanceOrg run fail. Please check docker log.'
else
    echo 'peer0.carMaintenanceOrg is active.'
fi
